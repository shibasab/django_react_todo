...
- Given 他ユーザーの繰り返しタスクIDを指定して更新操作する, When APIを呼び出す, Then 対象タスクは存在しないものとして 404 Not Found で拒否される。
...
- FR-003: タスク一覧および編集画面で、繰り返し設定の有無と種別を識別できなければならない。繰り返し設定が「なし」の場合、一覧での表示は省略する (MUST)。
...
- NFR-001: 繰り返しタスクの完了操作後、クライアントは完了操作API成功後の再取得結果に基づいて、次回タスク生成の結果を一覧へ反映しなければならない (MUST)。
