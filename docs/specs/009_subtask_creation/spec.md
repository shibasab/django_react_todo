...
- サブタスクに繰り返し設定（daily/weekly/monthly）を指定した場合、作成・更新が拒否されること
- 繰り返し親タスクの完了で次回タスクが生成される場合、サブタスクは引き継がれず新しい親タスクは子0件で開始されること
...
- FR-011: システムはサブタスク（`parent_id != NULL`）に対して繰り返し設定（`daily`/`weekly`/`monthly`）を許可してはならない（MUST）。
- FR-012: システムは繰り返し親タスクの次回タスク生成時に、サブタスクを複製してはならない。次回タスクはサブタスク0件で開始しなければならない（MUST）。
...
- D-005: 繰り返し設定は親タスクのみ許可し、サブタスクは `recurrenceType=none` 固定とする。
- D-006: 繰り返し親タスクの次回生成ではサブタスクを引き継がない。

## Open Questions

- なし（2026-02-21時点で、親のみ繰り返し許可・子は繰り返し不可で仕様固定）。
